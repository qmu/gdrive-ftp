...
func TestReadConcatenatesSegmentsOldestFirst(t *testing.T) {
	...
	writeSeg := func(p, name string) {
		l := New(p)
		if err := l.Record(ctx, Entry{Op: OpMkdir, Name: name}); err != nil {
			t.Fatal(err)
		}
	}
...
func TestReadSkipsCorruptLineAndMissingFile(t *testing.T) {
	...
	content := `{"time":"2026-06-18T12:00:00Z","op":"trash","name":"a","id":"1"}` + "\n" +
		`{"time":"2026-06-18T12:01:00Z","op":"uploa` // truncated
...
func TestVerb(t *testing.T) {
	cases := map[Operation]string{OpUpload: "uploaded", OpTrash: "trashed", OpMkdir: "created", Operation("x"): "x"}
