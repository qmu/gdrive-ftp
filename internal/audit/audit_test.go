...
func TestEntryMarshal(t *testing.T) {
	...
	for _, absent := range []string{"driveId", "cwd"} {
...
func TestRecordAppendsAndPermissions(t *testing.T) {
	...
	if err := l.Record(ctx, Entry{Op: OpMkdir, Name: "Work", ID: "f1"}); err != nil {
		t.Fatal(err)
	}
	if err := l.Record(ctx, Entry{Op: OpTrash, Name: "old.pdf", ID: "f2"}); err != nil {
		t.Fatal(err)
	}
	...
	if len(ops) != 2 || ops[0] != OpMkdir || ops[1] != OpTrash {
		t.Fatalf("ops = %v, want [mkdir trash]", ops)
	}
...
func TestRecordNilLoggerIsNoOp(t *testing.T) {
	var l *Logger
	if err := l.Record(context.Background(), Entry{Op: OpMkdir, Name: "x"}); err != nil {
...
func TestRecordRotatesAtCap(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		if err := l.Record(ctx, Entry{Op: OpMkdir, Name: "folder-with-a-longish-name", ID: "id"}); err != nil {
			t.Fatal(err)
		}
	}
