// Package audit records mutating Google Drive operations (upload, trash, mkdir)
// to an append-only JSON Lines log so a user or AI agent can look back at — and
// recover from — changes the CLI made. The log is owned data: it never contains
// credentials or file contents, only the identity of each change. Writes are
// best-effort; a logging failure must never break the operation that triggered
// it.
package audit
...
// Operation is the kind of Drive mutation an Entry records. The values mirror
// the CLI's own action vocabulary.
type Operation string

const (
	OpUpload Operation = "upload" // a put: new file or content overwrite
	OpTrash  Operation = "trash"  // an rm: file/folder moved to the trash
	OpMkdir  Operation = "mkdir"  // a folder created
)

// Entry is one append-only audit record. It captures what changed — the target
// and, for an overwriting put, the before/after size — never the file contents
// and never any credential. Zero-valued optional fields are omitted.
type Entry struct {
	...
	Replaced  bool      `json:"replaced,omitempty"`  // a put that overwrote an existing file
	PriorSize int64     `json:"priorSize,omitempty"` // the overwritten file's size, before
}
...
// New returns a Logger that appends to path (e.g. ~/.config/gdrive-ftp/audit.jsonl).
